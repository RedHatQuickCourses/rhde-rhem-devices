...
.. May need to add a subscription for the RHEM packages, to get the RHEM agent during bootc container builds
...
Now your _development machine_ ready to build bootc container images with RHEL 9 for edge devices, run local VMs as virtual edge devices, and access a Red Hat Edge Manager server which you will configure in this course and will manage those virtual edge devices.
