...
* Other teams may opt for a strict separation of concerns, where an infrastructure team manages operating system configurations and hardware lifecycle, while other teams manage application lifecycle and data.
...
It is not a rigid approach of all managed devices must be configured in exactly the same way, but it is flexible enough to address small variations in a centralized manner, for example: each edge site contains its own HTTP proxy server and all PoS systems must use the proxy from their own site.
...
If you opt for deploying RHEM integrated with either ACM or AAP, RHEM looks like it is part of those platforms, integrated to their web interfaces, their API routing, and using their authentication and authorization mechanisms, but still offering its own features and APIs.
...
Individual managed devices run an RHEM agent, similar to a kubelet, which observes configuration changes from the API server and state changes from the device itself, and reconciles them if there is any difference.
