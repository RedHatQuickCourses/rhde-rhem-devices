= Deploying Red Hat Edge Manager and Managing Edge Devices
:navtitle: Home

== Goal
This course teaches the role of Red Hat Edge Manager (RHEM) by itself and relative to other infrastructure management tools from Red Hat. This course also teaches how to deploy RHEM on Red Hat Enterprise Linux (RHEL) servers from RPM packages; and enroll, onboard, and manage individual edge devices running Red Hat Device Edge and image mode for RHEL.

It is the first in a series of quick courses about Red Hat Edge Manager (RHEM), which will consist of two courses:

. *https://redhatquickcourses.github.io/rhde-bootc-build/[*Deploying Red Hat Edge Manager and Managing Edge Devices* (this course)

. https://TBD[Managing Edge Device Fleets and Applications with Red Hat Edge Manager] (follow-up course)

IMPORTANT: Red Hat associates must enroll using the https://training-lms.redhat.com/sso/saml/auth/rhlpint?RelayState=deeplinkoffering%TBD[Red Hat Learning Portal], and Red Hat Partners must enroll using the https://training-lms.redhat.com/sso/saml/auth/rhopen?RelayState=deeplinkoffering%TBD[Red Hat Partner Connect].
Red Hat employees and partners must also enroll on HOL019 by following the links in the _Classroom environment_ heading bellow
...
* Deploy RHEM standalone on RHEL.
...
Red Hat and Partner roles that support customers of Red Hat Edge Manager, such as Services Consultants and Customer Support Engineers.
...
* Familiarity with image mode for RHEL, especially building bootc container images and provisioning and updating image mode systems It's recommended that learners already process skills from the https://redhatquickcourses.github.io/rhde-bootc-build/[image mode with Red Hat Device Edge quick course series].
Notice that no prior knowledge of Kubernetes and OpenShift is assumed nor required.

== Classroom environment

You can perform all activities in this course using the virtual classroom lab from *HOL019: Image Mode with Red Hat Device Edge Quick Course Series*, in the Red Hat Online Learning Environment (ROLE), which is shared with thequick courses about image mode with Red Hat Device Edge and will be updated in the future to better support RHEM and other RHDE training.

IMPORTANT: The RHEM quick courses use ROLE for their virtual classroom labs only, and hosts course materials outside of ROLE, as GitHub pages.
...
* https://training-lms.redhat.com/sso/saml/auth/rhopen?RelayState=deeplinkoffering=84932768[HOL019 for Red Hat partners^] on the Red Hat Partner Connect
...
For documentation about RHEL image mode, see either the https://docs.redhat.com/en/documentation/TBD[RHEM product documentation^] or its https://github.com/flightctl/flightctl/tree/main/docs/user[upstream project documentation^].
...
//Special thanks to <the flightctl team>.
