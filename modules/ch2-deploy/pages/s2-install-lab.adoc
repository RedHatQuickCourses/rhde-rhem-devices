...
= Lab: Install Red Hat Edge Manager on RHEL

_Estimated reading time: *{time_estimate} minutes*._

Objective::

Deploy Edge Manager on RHEL with minimal requirements.
...
Your _development machine_ is already registered to Red Hat with a valid Red Hat Device Edge subscription by the xref:ch1-intro:s3-classroom-lab.adoc[first lab].
...
You will install and configure Edge Manager on your _server machine_, configure a couple users with different roles, and then you will use your _development machine_ to run the web UI and the CLI of Edge Manager and verify that your new Edge Manager instance is healthy.

NOTE: The instructions here use the community artifacts from the Flight Control project, but are expected to work with the product artifacts from Edge Manager, when they become available, by just switching from the community repository to the Edge Manager product repository and using the Red Hat subscription manager to register your _server machine_.
...
Cannot use the current RHEM artifacts with RHEL 10, their containers fail signature check. Have to keep using Fight Control upstream artifacts or create a classroom with RHEL 9.6 servers (which I also need for MicroShift)
////
...
.. Other preparation tasks performed during the xref:ch1-intro:s3-classroom-lab.adoc[first lab] of this course are not required yet, because most of this lab happens on the _server machine_, but they will be necessary for the remaining labs.

2. Install the Edge Manager services on your _server machine_.
...
.. Install the container tools on your _server machine_ because the Edge Manager packages just configure Systemd to run the Edge Manager services as quadlets, that is, as Podman containers.
+
[source,subs="verbatim,quotes"]
--
...
You need this because the Edge Manager RPM packages need to download their container images.
+
[source,subs="verbatim,quotes"]
--
...
--
+
NOTE: If you want to try the current latest release from the upstream project, remove the version component `-1.0.0` from the package `flightctl-services` in the command above.
+
////

.. Edit the Edge Manager configuration in `/etc/flightctl/service-config.yaml` to use the fully qualified domain (FQDN) of your _server machine_.
+
...
Tested on a RHEL 9.7 VM (installed as 9.6 then updated to 9.7) and Edge Manager packages just work. Too bad it's not an environment I can easily add virtual edge devices. And even worse it's not something I can easily replicate on ROLE: I'd have to build a new custom classroom (a new HOLxxx) which adds days/weeks to my schedule. :-(
////
...
.. Verify that the Edge Manager containers are up and running.
+
[source,subs="verbatim,quotes"]
--
$ *sudo podman ps*
...
.. You can also get logs directly from the containers, or enter them with `podman exec` for deeper troubleshooting.
+
[source,subs="verbatim,quotes"]
--
...
3. For simplicity, configure local users for the Edge Manager services, on the PAM issuer container.
...
.. If you would rather be more thorough verifying users and groups on the PAM issue container, use the following commands.
+
[source,subs="verbatim,quotes"]
--
...
4. Install the Edge Manager CLI on your _development machine_
...
5. Access the Edge Manager web UI.
...
.. Log in as the `student` user, with the password `student`. 
+
NOTE: If you are iddle for too long on the login page, you may get a "CSRF error".
If it happens, just try again.

.. You should get the Edge Manager menu:Overview[] page, with three gray circles because there are no managed devices on which to report a status summary.

.. On the left panel, click menu:Devices[] to verify that there are indeed no managed devices.

== What's next

In the next activity, you will create a bootc container image containing the Edge Manager client and use it to provision and enroll a test VM which stands in for a managed edge device.