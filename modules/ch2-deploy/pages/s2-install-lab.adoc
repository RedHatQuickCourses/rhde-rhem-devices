...
2. Install the RHEM services on your _server machine_.
...
Tested on a RHEL 9.7 VM (installed as 9.6 then updated to 9.7) and RHEM packages just work. Too bad it's not an environment I can easily add virtual edge devices. And even worse it's not something I can easily replicate on ROLE: I'd have to build a new custom classroom (a new HOLxxx) which adds days/weeks to my schedule. :-(
////
...
3. For simplicity, configure local users for the RHEM services, on the PAM issuer container.
...
4. Install the RHEM CLI on your _development machine_
