...
RHEM agents also invoke the RHEM API, but using a different entrypoint than the CLI and web UI do, and authenticate using a certificate generated at device enrollment time.
...
If you must perform RHEM datadatabase backups and other database management operations,
you must manage Podman volumes or enter container namespaces of RHEM containers.
