...
Dec 17 21:44:59 enroll flightctl-agent[1194]: https://172.25.250.10:443/enroll/{edge-1-uuid-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa}
--
+
TODO: REVIEW IF THE URL ABOVE CHANGES TO USE THE FQDN
...
[source,subs="verbatim,quotes,attributes"]
--
$ *flightctl approve enrollmentrequest _{edge-1-uuid-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa}_ -l alias=edge-1 -l rhel=9.6 -l type=noapp*
GRAB OUTPUT
--
...
$ *flightctl get enrollmentrequest*
NAME                                                    APPROVAL        APPROVER        APPROVED LABELS
{edge-1-uuid-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa}    Approved        admin           alias=edge-1,rhel=9.6,type=noapp
--
...
[source,subs="verbatim,quotes,attributes"]
--
$ *sudo journalctl -u flightctl-agent | grep Bootstrapping*
...
.. Type `edge-2` as tha alias of the device and click on *Add label* to show a text field where you will type the key and value for a single label.
+
GRAB SCREENSHOT
...
8. Verify that, despite being provisioned from the same enrollment request data, each test VM retains its own identity and configuration from RHEM.
