...
--
+
NOTE: You will login again on both the Red Hat registry and your private registry as the root user, because different Linux users do not share container registry authentication tokens.

.. Verify that you have administrator access to a RHEM server.
+
[source,subs="verbatim,quotes"]
--
...
2. Create a bootc container image which includes the RHEM agent.
...
--
+
NOTE: If you want to try with the current lastest release of the upstream project, edit your Containerfile to remove the version component `-1.0.0` from the package `flightctl-agent`.
...
Notice that the `podman build` command finnishes with a period, for the current directory.
+
...
+
The script expects a single digit as an argument, which is used to compose both the name of the VM and the local port to forward do the SSH port of the VM.
+
...
.. If you need to retry creating your test VM, use the following command and return the step that copies the generated ISO image.
+
[source,subs="verbatim,quotes"]
--
$ *virsh destry edge-1*
...
.. Notice that, in the begining of the RHEM agent logs, it generates a certificate signing request (CSR) for its own individual enrollment request with the RHEM server, and its own unique UUID.
+
...
.. Also notice that the agent keeps polling th RHEM server for its enrollment request to be approved.
+
[source,subs="verbatim,quotes"]
--
...
.. The agent logs also provide a URL and a QR code (not shows in the output bellow) which you can use to approve the certificate request for the device
+
[source,subs="verbatim,quotes,attributes"]
--
...
.. Verify that there is an enrollment request with an UUID which corresponds to the one in the logs of your test VM, but no devices.
+
[source,subs="verbatim,quotes,attributes"]
--
...
6. Verify that the test VM is now managed by RHEM.
...
.. If you did not set the correct labels, during approval, you can use the following command do edit the device resource and add the three entries under `labels:` and the previous listing as a guide.
+
[source,subs="verbatim,quotes,attributes"]
--
...
.. The flightctl CLI does not provide built-in filtering, but you can pipe its JSON output to the `jq` utility and filter just the information you need, for example:
+
[source,subs="verbatim,quotes,attributes"]
--
...
.. On the managed device, verify that the logs RHEM agent record that the device was enrolled and synchronized, that is, reconcilled with its currently empty configuration in the RHEM server.
+
[source,subs="verbatim,quotes"]
--
...
.. Create a file on the managed device, just so you can easily distinguish it from other managed devices you create during this course.
+
[source,subs="verbatim,quotes"]
--
...
.. On your second test VM, verify that it generated a different UUID to identify itself to RHEM and return to your host shell.
+
[source,subs="verbatim,quotes,attributes"]
--
...
8. Enroll your second test VM on RHEM, but this time using its web interface and a non-administrator user.

.. Open a web browser and visit `\https://servera.lab.example.com`.

.. Log in as the `admin` user with the `redhat` password.
Notice we are NOT using the `student` user, because you need full administrator privileges on RHEM to approve managed devices.
+
GRAB SCREENSHOT
...
.. Scroll back to the begining of the page to approve your second test VM as a managed device.
+
Click on the *Untitled* link on the *Alias* column.
...
.. Return to the *Devices* page and click on the *Approve* link to the right.
+
GRAB SCREENSHOT

.. Type `edge-2` as the alias of the device and click on *Add label* to show a text field where you will type the key and value for the next label.
+
GRAB SCREENSHOT

.. Type `rhel=9.6` and press kbd:[Tab] to display the *Add label* link.
...
.. The warning *No fleet is matching the selected labels* is expected and can be safely ignored. 
Click *Approve*.
+
GRAB SCREENSHOT

.. The *Devices* page now shows no devices pending approval, and shows the new managed device with an *Out-of-date* status.
+
GRAB SCREENSHOT

.. Wait a few moments until the device synchronizes with RHEM and its status changes to *Up-to-date*.
+
GRAB SCREENSHOT

9. Verify that, despite being provisioned from the same enrollment request data, each test VM retains its own identity and configuration from RHEM.

.. On the *Devices* page, click on the *edge-2* alias to enter the manged device details page of your second test VM.
+
GRAB SCREENSHOT

.. Click on *YAML* to see the resource manifest of the device, like you would get from the `flightctl get device -o yaml` command.
+
GRAB SCREENSHOT

.. Click on *Terminal* to get a shell on the managed device, like you would get from the `flightctl console` command.
+
GRAB SCREENSHOT

.. Verify that the second test VM does not contains a `device.txt` file.
+
GRAB SCREENSHOT

You now have two test VMs enrolled in RHEM as managed devices.
You also know how to identify each one and saw that you can access these devices, to run troubleshooting commands, directly through RHEM.
