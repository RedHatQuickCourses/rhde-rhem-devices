...
.. Enter the `test-enroll` directory, which contains the SSH key used to access the two test VMs.
+
[source,subs="verbatim,quotes"]
--
$ *cd temp-enroll*
...
+
////
It seems my test VMs are consitently offline, and don't connecto to RHEM, when I continue the next day.
...
[IMPORTANT]
====
If it seems that your test VMs are staying for too long in the `Unknow` state, access them using SSH and verify if their clocks are accurate by using the `date -u` command and comparing the output with the same command on your _development machine_.
...
The clock skrew prevents them from synching with RHEM.
