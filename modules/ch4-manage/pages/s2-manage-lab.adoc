...
Configure managed devices by using inline configuration in the device specification[, and update the operating system image on managed devices].
...
//Right now, they have identical settings, and you must distinguish them by indirect means, such as files that you created on each device, or the local port forwarded to each test VM.
...
//You will remediate this by configuring your managed devices with basic settings, such as hostname and other configuration files in the `/etc` directory, as inline configurations in each device manifest.
...
// This is for the second lab, if I decide to split
...
88. Need another simple scenario to replace the hostname change.
Anything easy to see in a basic RHEL, which wouldn't require adding an HTTP server and other services to the bootc image?

99. Change the TimeZone to show that you can still make changes on non-managed files and RHEM would not interfere with them.
