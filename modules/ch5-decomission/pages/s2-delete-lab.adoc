...
.. Notice that there is no action do either decomission or delete a device, but only to edit device configurations and view device details.
+
...
.. You could keep the device as being decomissioned for as long as you wish, but after a time you probably want to remove its records from the RHEM database.
+
...
6. Decomission another device, but now using the CLI.
...
8. Verify that your test devices cannot communicate with their RHEM server anymore.
+
////
Behavior changed from RC to 1.0.0. Devices used to attempt to re-enroll, now they don't.
////

.. Open a SSH session to one of your test VMs and shutdown it.
+
[source,subs="verbatim,quotes"]
--
$ *ssh -i edge-2-key -p 8222 core@127.0.0.1*
...
+
Verify that the device believes it is already enrolled with RHEM, but it cannot update its status on the RHEM server.
+
[source,subs="verbatim,quotes,attributes"]
-- 
$ *ssh -i edge-2-key -p 8222 core@127.0.0.1*
